...
import com.nexysquare.ddoyac.env.Utils;
...
//        d.ANCHORS = anchors;
        ...
        d.numClass = numClass;
        d.outData = ByteBuffer.allocateDirect(d.output_box * (numClass) * numBytesPerChannel);
...
//        float[][][] outbuf = new float[1][output_box][labels.size() + 5];
        ...
        for (int i = 0; i < output_box; ++i) { // 320 -> 6300번
        ...
        for (int i = 0; i < output_box; ++i){
            ...
            if (confidenceInClass > getObjThresh()) {
                LOGGER.d("testtest maxClass:%f, confidence:%f, confidenceInClass:%f, detectedClass:%d", maxClass, confidence, confidenceInClass, detectedClass);
                LOGGER.d("testtest box:  x:%f, y:%f, w:%f, h:%f", out[0][i][0], out[0][i][1], out[0][i][2], out[0][i][3]);
                ...
                LOGGER.d("testtest detections:$s", detections.toString());
            }
        }
